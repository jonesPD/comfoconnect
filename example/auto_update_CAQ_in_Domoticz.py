...
def domo_restart():
	...
	print('Could not connect to Domoticz server ' + DOMO['SERVER'] + '. Trying to restart Domoticz...')
...
def get_power(pdid):
	...
	if pdid == SENSOR_AVOIDED_COOLING_TOTAL: rvalue = CAQ_pdid[SENSOR_AVOIDED_COOLING_CURRENT]
	if pdid == SENSOR_AVOIDED_COOLING_CURRENT: rvalue = CAQ_pdid[SENSOR_AVOIDED_COOLING_TOTAL]

	if rvalue == None:
		if pdid == SENSOR_POWER_CURRENT or pdid == SENSOR_AVOIDED_HEATING_CURRENT or pdid == SENSOR_AVOIDED_COOLING_CURRENT:
...
def callback_sensor(pdid, value):
	...
	# AVOIDED COOLING
	if pdid == SENSOR_AVOIDED_COOLING_CURRENT:
		## Update Avoided ; ENERGY can be either from device or computer by Domoticz (configurable through 'edit' of device)
		# /json.htm?type=command&param=udevice&idx=IDX&nvalue=0&svalue=POWER;ENERGY
		Precision = 10
		#Read out value as shown in domoticz to average out value and prevent 'continuous updates'
		nvalue = round(round((value + CAQ_pdid[pdid])/2)/Precision)*Precision
		CommandValue = 'nvalue=0&svalue='+str(round(nvalue/Precision)*Precision)+';'+str(get_power(pdid)*1000)
		LogValue = 'current Avoided Cooling to ' + str(round(nvalue/Precision)*Precision) + 'W (cumulative: ' + str(get_power(pdid)) + 'kWh).'
	if pdid == SENSOR_AVOIDED_COOLING_TOTAL:
		## Update Avoided ; ENERGY can be either from device or computer by Domoticz (configurable through 'edit' of device)
		# /json.htm?type=command&param=udevice&idx=IDX&nvalue=0&svalue=POWER;ENERGY
		Precision = 10
		nvalue = value
		#Read out present cumulative power consumption
		CommandValue = 'nvalue=0&svalue='+str(round(get_power(pdid)/Precision)*Precision)+';'+str(value*1000)
		LogValue = 'cumulative Avoided Cooling to ' + str(value) + 'kWh (current: ' + str(round(get_power(pdid)/Precision)*Precision) + 'W).'
	...
	if LogValue == None: 
		...
		try:
			...
			if CAQ_pdid[pdid] != nvalue or time.time() > CAQ_pdid_time[pdid]:
				if pdid == SENSOR_OPERATING_MODE_BIS or pdid == SENSOR_FAN_SPEED_MODE:
					LogValue += ' ' + domo_setSelector(idx, nvalue)
				elif CommandValue != None:
					LogValue += ' ' + domo_updatedevice(CommandValue, idx)
				else: 
					LogValue = 'NO CommandValue specified: ' + LogValue
...
def infinite_loop():
	...
	comfoconnect.register_sensor(SENSOR_AVOIDED_COOLING_CURRENT)  # Avoided Cooling: Avoided actual
	comfoconnect.register_sensor(SENSOR_AVOIDED_COOLING_TOTAL)  # Avoided Cooling: Avoided total
	...
	reconnect_timer = 0
	try:
		while domo_online() and reconnect_timer < 10:
			...
			if not comfoconnect.is_connected():
				print('We are not connected anymore...trying to reconnect...'+str(reconnect_timer))
				reconnect_timer += 1
				#Add counter, if >10x, exit!
			else:
				reconnect_timer = 0
				domo_MODE = domo_getModeSelector()
				if get_mode(CAQ_pdid[SENSOR_OPERATING_MODE_BIS]) != domo_MODE:
					print('Request for changing mode detected (domo_MODE='+domo_MODE+'; CAQ_pdid='+get_mode(CAQ_pdid[SENSOR_OPERATING_MODE_BIS])+')...Go to mode: '+ domo_MODE)
					if domo_MODE == 'MANUAL':
				...
				if get_speed(CAQ_pdid[SENSOR_FAN_SPEED_MODE]) != domo_SPEED: 
					print('Request for changing speed detected (domo_SPEED='+domo_SPEED+'; CAQ_pdid='+get_speed(CAQ_pdid[SENSOR_FAN_SPEED_MODE])+')...Go to speed: '+domo_SPEED)
					if domo_SPEED == 'AWAY':
